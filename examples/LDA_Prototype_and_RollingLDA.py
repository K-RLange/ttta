...
# also includes a change detection method. The data used here is a sample of
# the SpeakGer dataset, which contains speeches from German federal state
# parliaments as well as the German Bundestag
...
# function that includes tokenization.
# ----------------------------LDAPrototype----------------------------
...
# parameters in fit. Here, we train an LDA with 30 topics, 5 LDAs to be
# compared for LDAPrototype, and a minimum threshold of 5 (absolute) and
# 0.002 (relative) for topic matching in the prototype step.
...
# less than _min_docs_per_chunk, default 10 * K), then chunks are merged.
...
print(roll.doc_topic_matrix(1))
# and the word-topic matrix for a given chunk
print(roll.doc_topic_matrix(1))
# or for all chunks
print(roll.word_topic_matrix())
...
# For more information, call the help pages for each method via help(
# TopicalChanges) and if you are still unsure about something, find bugs,
