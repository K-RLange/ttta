...
class FLDA:
    def __init__(self, data: pd.DataFrame, topics_per_layer: List[int], std_dev_alpha: float = 1, std_dev_alpha_bias: float = 1, std_dev_omega: float = 0.5,
                 ...
                 verbose: int = 1, min_count: int = 2,) -> None:
        ...
        if len(meta_columns) > 0:
            self.doc_meta_mapping = data.apply(lambda row: ",".join(row[meta_columns]), axis=1).to_numpy()
            meta_lengths = []
            metas = []
        ...
        self.assignments = np.zeros(self._total_number_of_tokens, dtype=np.uint32)
        ...
        for index in range(self._total_number_of_tokens):  # todo only do this init if there are priors, otherwise sample completely randomly
    ...
    def _create_dtm(self, texts: List[List[str]], dtm: np.ndarray = None) -> None:
        """
        ...
        Args:
            texts: list of texts
            dtm: existing document-term matrix
        Returns:
            None
        """
        if self._verbose > 0:
            print("Creating document-term matrix...")
        self._dtm, self._vocab, self._deleted_indices = create_dtm(texts, self._vocab, self._min_count, self._deleted_indices, dtm)
        if self._verbose > 1:
    ...
    def update_weights_alpha(self):
        ...
        dg = np.vectorize(digamma_func)(self.alpha_norm) - np.vectorize(digamma_func)(self.alpha_norm + self.length_of_documents.reshape((1000,)))
        ...
        total_dg = dg.reshape((self.number_of_docs, 1)) + dgW
        gradient_likelihood = np.multiply(np.multiply(self.document_topic_prior, total_dg), self.document_meta_matrix)
        gradientB = np.sum(gradient_likelihood)
        gradientBeta = np.sum(gradient_likelihood * (1-self.logistic(self.beta)).reshape((self.K, 1)))
        gradient_document_topic = [np.zeros((z, self.number_of_docs)) for z in self.topics_per_layer]
        all_topic_locations = [self.get_topic_matrix_location(x) for x in range(self.K)]
        # todo so richtig? für jedes dokument ein eigener gradient_document_topic?
        for layer in range(self.number_of_layers):
            for topic_location_in_layer in range(self.topics_per_layer[layer]):
                gradient_topic[layer][topic_location_in_layer] += gradient_likelihood[:, [x[layer] == topic_location_in_layer for x in all_topic_locations]].sum()
                gradient_document_topic[layer][topic_location_in_layer] += np.ravel(gradient_likelihood[:, [x[layer] == topic_location_in_layer for x in all_topic_locations]].sum(axis=1))
        for layer in range(self.number_of_layers):
            self.alpha_local_factor_matrices[layer] += (gradient_document_topic[layer] - self.alpha_local_factor_matrices[layer] / (sigma ** 2)).sum(axis=0) * self.local_step_alpha
            for topic_location_in_layer in range(self.topics_per_layer[layer]):
                self.alpha_global_tuple_matrix[layer][topic_location_in_layer] += (gradient_topic[layer][topic_location_in_layer] - self.alpha_global_tuple_matrix[layer][topic_location_in_layer] / (sigma ** 2)) * self.global_step_alpha
    ...
    def do_sampling(self, iter):
        # todo outsource sampling into C
        for index in range(len(self._doc_vec)):
            if self.meta:
    ...
    def update_priors(self):
        for topic in range(self.K):
            ...
            alpha_weight = self.bias_alpha + self.alpha_global_tuple_matrix[[i for i in range(self.number_of_layers)], z].sum()
            all_docs_weight = np.full(self.number_of_docs, alpha_weight)
            for layer in range(self.number_of_layers):
                all_docs_weight += self.alpha_local_factor_matrices[layer][z[layer]]
                omega_weight += self.omega_topic_word[layer][z[layer]]
    ...
    def run(self, iterations, samples, filename):
    ...
    def log_likelihood(self):
        likelihood = np.log(np.matmul((self.document_topic_count + self.document_topic_prior) /
                                      (self.length_of_documents.transpose() + self.alpha_norm).reshape((self.number_of_docs, 1)) + 1e-12,
    ...
    def write_output(self, filename="flda.pickle"):
